...
import { HandThumbUpIcon, HandThumbDownIcon, TagIcon, CheckCircleIcon as CheckOutline } from '@heroicons/react/24/outline';
...
const MessageItem: React.FC<MessageItemProps> = ({ message, isCurrentUserMessage, currentUserVote, onVoteQuestion }) => {
                ...
                <ul className="list-disc list-inside pl-1 space-y-0.5">
                  {message.options.map(opt => (
                    <li key={opt.id} className={`text-sm flex items-center ${isCurrentUserMessage ? 'text-white' : 'text-gray-800 dark:text-gray-100'}`}>
                       {message.questionType === QuestionType.MULTIPLE_CHOICE_MULTIPLE && (

...
import { QuestionType, QuestionOption, MatchingItem } from '../types';
import { PlusCircleIcon, TrashIcon, PhotoIcon, XCircleIcon, TagIcon, CheckIcon, InformationCircleIcon } from '@heroicons/react/24/outline';
...
function formReducer(state: FormState, action: any): FormState {
  switch (action.type) {
    ...
    case 'REMOVE_IMAGE':
      if (state.imagePreviewUrl) URL.revokeObjectURL(state.imagePreviewUrl);
      return { ...state, selectedImageFile: null, imagePreviewUrl: null };
...
const QuestionModal: React.FC<QuestionModalProps> = ({ isOpen, onClose, onSubmit, groupName }) => {
      ...
      {state.options.map((option, index) => (
        <div key={option.id} className="flex items-center space-x-2">
          ...
          <input
            ...
            className="flex-1 p-2 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-blue-500 dark:focus:border-blue-400 shadow-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 placeholder-gray-400 dark:placeholder-gray-500"
            required
          />
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 dark:bg-opacity-75 flex items-center justify-center p-4 z-50 transition-opacity duration-300 ease-in-out" role="dialog" aria-modal="true" aria-labelledby="question-modal-title">
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-2xl transform transition-all duration-300 ease-in-out scale-100 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            <label htmlFor="questionStem" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Question Stem</label>
            <textarea
              id="questionStem"
              value={state.stem}
          ...
          <div className="mb-4">
            ...
            {state.selectedImageFile && <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Selected: {state.selectedImageFile.name}</p>}
          </div>
